...
bool CDirectEngine::Init(const CEngine& aEngine, Vector2<unsigned int> aWindowSize, bool enableVSync, bool startInFullScreen)
{
    ...
    if( CollectAdapters( aWindowSize, numDenum ) )
    {
	...
	if (!myDeviceContext)
	{
		ERROR_AUTO_PRINT("%s", "Device context error");
	}


    SetDebugObjectName( myDeviceContext, "myDeviceContext" );

    if( FAILED( result ) || !mySwapchain )
    {
        ERROR_AUTO_PRINT( "%s", "Device swap error" );
        return false;
    }

    d3dDebug = nullptr;
#ifdef _DEBUG
    if( SUCCEEDED( myDevice->QueryInterface( __uuidof( ID3D11Debug ), (void**)&d3dDebug ) ) )
    {
    ...
    result = myDevice->CreateBlendState( &blendStateDescription, &m_alphaEnableBlendingState );
    if( FAILED( result ) )
    {
        return false;
    }
    ...
    myDeviceContext->OMSetBlendState( m_alphaEnableBlendingState, blendFactor, 0xffffffff );
