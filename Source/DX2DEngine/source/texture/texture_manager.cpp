...
CTexture* CTextureManager::GetTexture(const char* aTexturePath)
{
	...
	HRESULT hr = DirectX::CreateDDSTextureFromFile(myDirect3D->GetDevice(), DX2D::ConvertCharArrayToLPCWSTR(aTexturePath), nullptr, &resource);

	if (FAILED(hr))
	{
		hr = DirectX::CreateWICTextureFromFile(myDirect3D->GetDevice(), myDirect3D->GetContext(), DX2D::ConvertCharArrayToLPCWSTR(aTexturePath), nullptr,
			&resource);
		if (!FAILED(hr))
		{
